...
readonly ARCHIVE="${DESTDIR}/GeoIP-Anonymous-Plus.tar.gz"
...
curl --silent --fail --output "${ARCHIVE}" --time-cond "${ARCHIVE}" --location "https://download.maxmind.com/app/geoip_download?edition_id=GeoIP-Anonymous-Plus&license_key=${MAXMIND_LICENSE}&suffix=tar.gz" || {
  echo "${BASH_SOURCE[0]##*/}: fatal error: download failed." >&2
  exit 1
}

[[ "${LOCAL_TIMESTAMP}" = "$(date +'%s' --reference="${ARCHIVE}" 2>/dev/null)" ]] || {
  readonly TMPFILE="$(mktemp -p "${DESTDIR}" -t 'GeoIP-Anonymous-Plus.mmdb.XXXXXXXXXXXXXXXX')"

  tar --extract --directory "${DESTDIR}" --strip-components=1 --wildcards '*.mmdb' \
    --transform='flags=rSH;s|GeoIP-Anonymous-Plus\.mmdb$|'"${TMPFILE##*/}"'|g' --file "${ARCHIVE}" || exit 1

  cat "${TMPFILE}" >"${DESTDIR}/GeoIP-Anonymous-Plus.mmdb"
}
